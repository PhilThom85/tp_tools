...
class AssignUnlessDefTest < Test::Unit::TestCase
  ...
  must "get value from array and assign correctly" do
    ...
    l,w,h = foxy_set [ :length, :width, :height ], :default => 1, :array => arr
    assert_equal 5, l
    assert_equal 18, w
    assert_equal 7, h
  end
  ...
  must "get value from array and assign correctly if key does not exist" do
    ...
    l,w,h = foxy_set [ :length, :width, :height ], :default => 666, :array => arr
    assert_equal 5, l
    assert_equal 666, w
    assert_equal 7, h
  end
  ...
  #   l,w,h = foxy_set [0,1,2], :array => verbose, :cond => ~> { |k| verbose.count >= k+1 }
